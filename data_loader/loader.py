...
# --------- make stdout/stderr UTF-8 friendly on Windows (best effort) ----------
try:
...
def _fallback_ccxt_ingest(symbols: List[str], timeframe: str, since_utc: Optional[str], limit: int):
    ...
    for s in tqdm(symbols, desc="Ingest(ccxt)"):
        if s not in ex.markets:
            print(f"[ingest] skip {s}: not in exchange markets")
            continue
...
def step_ingest(symbols_arg: str, timeframe: str, since_utc: Optional[str], limit: int):
    ...
    if HAS_LOADER:
        ...
        #  d) load_ohlcv(...) / download_ohlcv(...) / get_ohlcv_df(...)  (additional aliases)
        if hasattr(_loader, "get_ohlcv"):
            ...
            ohlcv = pd.concat(frames, ignore_index=True).sort_values(["symbol", "ts"])
            _save_parquet(ohlcv, RAW_DIR / "ohlcv.parquet")
        elif hasattr(_loader, "load_ohlcv"):
            ohlcv = _loader.load_ohlcv(symbols=symbols, timeframe=timeframe, since_utc=since_utc, limit=limit)
            if not isinstance(ohlcv, pd.DataFrame) or ohlcv.empty:
                raise SystemExit("loader.load_ohlcv returned empty result")
            _save_parquet(ohlcv, RAW_DIR / "ohlcv.parquet")
        elif hasattr(_loader, "download_ohlcv"):
            ohlcv = _loader.download_ohlcv(symbols=symbols, timeframe=timeframe, since_utc=since_utc, limit=limit)
            if not isinstance(ohlcv, pd.DataFrame) or ohlcv.empty:
                raise SystemExit("loader.download_ohlcv returned empty result")
            _save_parquet(ohlcv, RAW_DIR / "ohlcv.parquet")
        elif hasattr(_loader, "get_ohlcv_df"):
            ohlcv = _loader.get_ohlcv_df(symbols=symbols, timeframe=timeframe, since_utc=since_utc, limit=limit)
            if not isinstance(ohlcv, pd.DataFrame) or ohlcv.empty:
                raise SystemExit("loader.get_ohlcv_df returned empty result")
            _save_parquet(ohlcv, RAW_DIR / "ohlcv.parquet")
        else:
